在以太坊链上, 没有什么是私有的. private 关键词只是 solidity 中人为规定的一个结构. 控制台的 `getStorageAt(address, slot)` 可以读取 storage 中的任何信息, 虽然有些数据读取的时候会比较麻烦. 因为 一些优化的技术和原则, 这些技术和原则是为了尽可能压缩 storage 使用的空间.
